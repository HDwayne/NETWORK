    ...
    class FileTransmissionProtocol:
        ...
        def _connect(self):
            ...
            for attempt in range(5):
                try:
                    self._sock.connect((self.client.server_address, self.client.server_port))
                    print("[FileTransmissionProtocol] Connecté avec succès au serveur pour le transfert de fichier.")
                    break
                except socket.error as e:
                    print(f"[FileTransmissionProtocol] Échec de la connexion au serveur, tentative {attempt + 1}/5")
                    if attempt < 4:
                        time.sleep(2)
                    else:
                        print("[FileTransmissionProtocol] Impossible de se connecter au serveur pour le transfert de fichier.")
                        exit(1)

        def _disconnect(self):
            if self._sock:
                self._sock.close()
                print("[FileTransmissionProtocol] Déconnecté du serveur de transfert de fichier.")
        ...
        def send_file(self, file_path):
            ...
            self._segments_to_send = [segments[i:i+self._segment_size] for i in range(0, len(segments), self._segment_size)]
            
            self._current_base = 0
            seq_num = 0
            while seq_num < len(self._segments_to_send):
                with self._retransmission_lock:
                    if not self._retransmission_in_progress and seq_num <= self._current_base + self._window_size - 1:
                        self._send_data(self._segments_to_send[seq_num], seq_num)
                        seq_num += 1
                        setattr(self, f"_timer_{seq_num}", threading.Timer(self._timeout, self._check_timeout, args=(seq_num,)))
        ...
        def _check_timeout(self, seq_num):
            with self._retransmission_lock:
                if seq_num > self._current_base and not self._retransmission_in_progress:
                    ...
                    end_window = min(self._current_base + self._window_size, len(self._segments_to_send))
    ...
    class FileListRequestProtocol:
        ...
        def _connect(self):
            ...
            for attempt in range(5):
                try:
                    self._sock.connect((self.client.server_address, self.client.server_port))
                    print("[FileListRequestProtocol] Connecté avec succès au serveur pour la requête de liste de fichiers.")
                    break
                except socket.error as e:
                    print(f"[FileListRequestProtocol] Échec de la connexion au serveur, tentative {attempt + 1}/5")
                    if attempt < 4:
                        time.sleep(2)
                    else:
                        print("[FileListRequestProtocol] Impossible de se connecter au serveur pour la requête de liste de fichiers.")
                        exit(1)

        def _disconnect(self):
            if self._sock:
                self._sock.close()
                print("[FileListRequestProtocol] Déconnecté du serveur pour la requête de liste de fichiers.")
                ...
                if message_length_bytes:
                    ...
                    try:
                        ...
                        if response_message.type == 'LIST_RESPONSE':
                            print(f"[LIST  ] Received list response: {response_message.content}")
                    except Exception as e:
